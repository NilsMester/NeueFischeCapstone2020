...
import {addRecord, getUserRecordList, removeRecord, updateRecord} from "../service/RecordService";
...
export default function RecordContextProvider({children}) {
    ...
    useEffect(() => {
        tokenIsValid() && getUserRecordList(token)
            .then(setRecords)
            .catch(console.log);
    }, [token, tokenIsValid]);

    const createRecord = (titel, recordLink, description, publicStatus, tagList) =>
    ...
    const editRecord = (id, titel, recordLink, description, timestamp, publicStatus, tagList) =>{
        updateRecord(id, titel, recordLink, description, timestamp, publicStatus, tagList, token)
